...
class _ViewRequestsState extends State<ViewRequests> {
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
      ...
      body: new Container(
        child: requestsList.length == 0
            ? new Text("No Requests availabe")
            : new ListView.builder(
                itemCount: requestsList.length,
                itemBuilder: (_, index) {
                  return RequestsUI(
                      requestsList[index].building,
                      requestsList[index].customer,
                      requestsList[index].customerID,
                      requestsList[index].date,
                      requestsList[index].description,
                      requestsList[index].employeeName,
                      requestsList[index].flat,
                      requestsList[index].isSolved,
                      requestsList[index].phone,
                      requestsList[index].price,
                      requestsList[index].thumbnailUrl);
                },
              ),
      ),
    );
